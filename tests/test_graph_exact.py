...
def test_exact_optimum_is_verified_at_runtime(graph, exact):
    reference = exact["exact_reference"]
    route = tuple(reference["node_path"])
    assert route == (0, 1, 2, 4, 5, 6)
    assert reference["cost"] == 10
    assert path_cost(graph, route) == 10
    assert exact["unique_optimum"] is True
    assert exact["agreement"]["valid"] is True


def test_bit_order_and_route_decoding_round_trip(graph, exact):
    route = tuple(exact["exact_reference"]["node_path"])
